...
**Primary Topics:** Two Pointers | Left+Right Closing In (requires sort)

**Sorted?** Yes — must sort first. Sorted order gives elimination: sum too big → move right left; too small → move left right.
**Key:** Fix outer element k, then left+right converge on remaining sum. Sort also enables easy dedup: skip duplicate k, left, right values.
